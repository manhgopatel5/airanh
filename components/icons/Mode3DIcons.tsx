...
export const Briefcase3D = ({ active }: { active: boolean }) => (
  ...
  >
    ...
    <span className={`font-bold text-sm whitespace-nowrap ${active ? 'text-[#0A84FF]' : 'text-zinc-400'}`}>
      CÔNG VIỆC
    </span>
  </motion.div>
);
...
export const Palm3D = ({ active }: { active: boolean }) => (
  <div className="flex items-center gap-2">
    ...
    <span className={`font-bold text-sm whitespace-nowrap ${active ? 'text-[#30D158]' : 'text-zinc-400'}`}>
      GIẢI TRÍ
    </span>
  </div>
);