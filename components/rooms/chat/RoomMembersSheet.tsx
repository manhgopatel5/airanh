...
function RoomMembersSheet({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end">
      ...
      <div className="relative z-10 flex max-h-[75vh] w-full flex-col rounded-t-3xl border border-white/10 bg-zinc-900/95 backdrop-blur-xl animate-in slide-in-from-bottom-5">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          {loading ? (
            <div className="flex justify-center py-10">
              <FiLoader className="animate-spin text-white/60" size={24} />
            </div>
          ) : members.length === 0 ? (
            <p className="py-10 text-center text-sm text-white/50">Chưa có thành viên</p>
          ) : (
            <div className="space-y-1">
              {members.map((member) => (
                <div
                  key={member.uid}
                  className="flex items-center gap-3 rounded-xl px-2 py-2.5 active:bg-white/5"
                >
                  <img
                    src={member.photoURL}
