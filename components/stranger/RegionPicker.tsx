...
export default function RegionPicker({ value, onChange, className }: Props) {
  ...
  const setNationwide = () => {
    onChange(defaultStrangerRegion());
    setQuery("");
  };
    ...
    <div className={cn("space-y-3", className)}>
      <div className="grid grid-cols-2 gap-2">
        <motion.button
          ...
          onClick={handleGps}
          disabled={locating}
          className="flex h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 text-sm font-bold text-white disabled:opacity-60"
        >
          <FiMapPin size={16} />
        ...
        <motion.button
          ...
          className={cn(
            "flex h-12 items-center justify-center gap-2 rounded-xl border text-sm font-bold transition",
            value.province === NATIONWIDE
      ...
      <p className="text-xs leading-relaxed text-zinc-500">
        Chỉ hiển thị phường/xã và tỉnh khi match. Từ chối GPS thì thoát trang rồi vào lại để cấp quyền.
      </p>

      {value.displayLabel && value.province !== NATIONWIDE && (
