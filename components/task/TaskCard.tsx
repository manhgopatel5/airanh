...
const handleShare = useCallback(async () => {
  const shareUrl = `${window.location.origin}/task/${task.slug || task.id}`;
  const shareData = {
    title: task.title,
    text: `Xem task: ${task.title}`,
    url: shareUrl,
  };
 ...
 ? new Date(task.deadline.seconds * 1000).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' })
  ...
  {/* Nút Share - luôn hiện cho mọi user */}
  <motion.button
    whileTap={{ scale: 0.8 }}
    onClick={(e) => {
      e.stopPropagation();
