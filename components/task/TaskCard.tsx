        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2.5 flex-wrap">
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text- font-bold ${status.color}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${status.dot} animate-pulse`} />
                {status.label}
              </div>
              {isTask(task) && task.price > 0 && (
                <div className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-[#0A84FF]/10 to-[#0066CC]/10 dark:from-[#0A84FF]/20 dark:to-[#0066CC]/20 text-[#0A84FF] dark:text-[#8AB4F8] text- font-bold">
                  {task.price.toLocaleString("vi-VN")}đ
                </div>
              )}
              {task.viewCount && task.viewCount > 10 && (
                <div className="flex items-center gap-1 text- text-zinc-500 dark:text-zinc-400">
                  <FiEye size={12} />
                  <span>{task.viewCount}</span>
                </div>
              )}
            </div>
            <h3 className="font-bold text- text-zinc-900 dark:text-white line-clamp-2 leading-snug">
              {task.title}
            </h3>
          </div>
...
{onShare && (
  ...
  >
    <FiShare2 className="w-5 h-5 text-gray-400" />  
                    ...
                    <>
                      ...
                      >
                        <button
                          ...
                          className="flex items-center gap-3 px-4 py-2.5 text- text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700/50 w-full transition-colors"
                        >
                          <FiEdit2 size={16} /> Sửa
                        </button>
                        <button
                          onClick={handleDelete}
                          className="flex items-center gap-3 px-4 py-2.5 text- text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 w-full transition-colors"
                        >
        ...
        {/* Meta Info */}
        <div className="flex items-center gap-3 text- text-zinc-500 dark:text-zinc-400 flex-wrap">
          {taskDate && (
          ...
          <div className="flex items-center gap-1">
            <FiUsers size={13} />
            <span className="font-medium">{applicants.length}/{isTask(task)? task.totalSlots : 1}</span>
          </div>
          {task.location?.city && (
        ...
        <AnimatePresence>
          {isApplied && (
            ...
            >
              <div className={`flex items-center gap-1.5 text- ${themeColor.text} font-semibold`}>
