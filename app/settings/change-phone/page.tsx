          ...
          <Section title="DỮ LIỆU & LƯU TRỮ" delay={0.2}>
            ...
            <div className="px-5 py-4">
              ...
              <div className="h-2 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: `${(storage.used / storage.total) * 100}%` }} transition={{ duration: settings.reduceMotion? 0 : 0.8 }} className="h-full bg-gradient-to-r from-[#0042B2] to-[#1A5FFF] rounded-full" />
              </div>
            </div>
...
function SelectItem<T extends string>({ label, icon: Icon, value, options, onChange, reduceMotion }: SelectItemProps<T>) {
  ...
  return (
    <div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reduceMotion? 0 : 0.2 }} className="overflow-hidden bg-zinc-50 dark:bg-zinc-900/50">
            {options.map((opt) => (
              <button
                type="button"
                key={opt.value}
