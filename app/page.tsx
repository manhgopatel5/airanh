...
import {
  ...
  QueryDocumentSnapshot, DocumentData, where, Timestamp, doc,
  onSnapshot, updateDoc, serverTimestamp, getDoc, setDoc,
  deleteDoc, arrayUnion, arrayRemove, Unsubscribe, QuerySnapshot,
  writeBatch
...
// Icons
import { FiMapPin, FiSearch, FiMessageSquare, FiUserPlus, FiUsers, FiCheck, FiX, FiUpload, FiLoader, FiUserX, FiBell, FiAtSign, FiInbox } from "react-icons/fi";
...
import {
  ...
  Sparkles as SparklesIcon, CalendarRange, HelpCircle, LogOut,
  Trash2, Shield, Lock, Camera, Check, QrCode, Share2,
  ChevronRight, Settings, Circle, Zap, Star, ScanLine, X
} from "lucide-react";
...
type UserData = {
  uid: string; name: string; email: string; phone?: string; userId: string;
  avatar: string; bio?: string; online?: boolean; lastSeen?: Timestamp;
  createdAt?: Timestamp; emailVerified?: boolean; hidePhone?: boolean;
  stats?: { tasks: number; plans: number; completed: number; rating: number };
};

// ===== CHAT TYPES =====
type ChatItem = {
  uid: string; chatId: string; name: string; username: string; avatar: string;
  userId: string; lastMessage?: string; lastSenderId?: string; lastSenderName?: string;
  updatedAt?: Timestamp; isOnline?: boolean; unreadCount?: number; isTyping?: boolean;
  isGroup: boolean; members?: string[]; blockedUsers?: string[]; deletedFor?: string[];
};
type FriendItem = {
...
function ChatClient() {
  ...
  const handleScanFromFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (userId) {
        setSearch(userId); setAddMode("friend"); setShowAdd(true);
      ...
      try {
        await html5QrCode.start(
          ...
          (decodedText) => {
            ...
            if (userId) {
              setSearch(userId); setAddMode("friend"); setShowAdd(true);
              stopScan(); toast.success("Đã quét QR");
            } else { toast.error("Mã QR không hợp lệ"); }
          }, () => {}
        );
  ...
  //... (còn tiếp các hàm handleAddFriend, handleAccept... trong part 2)
// PART 2/3 - tiếp nối ChatClient
const handleAddFriend = useCallback(async (event?: React.FormEvent): Promise<void> => {
  ...
  try {
    ...
    await setDoc(doc(db, "friendRequests", requestId), {
      from: currentUser.uid, to: targetUserId, status: "pending", createdAt: serverTimestamp()
    });
...
const handleStartChatWithFriend = useCallback(async (friendId: string) => {
  ...
  const [currentUserDoc, friendDoc] = await Promise.all([
    getDoc(doc(db, "users", currentUser.uid)), getDoc(doc(db, "users", friendId))
  ]);
  const currentData = currentUserDoc.data(); const friendData = friendDoc.data();
  await setDoc(doc(db, "chats", chatId), {
    members: [currentUser.uid, friendId], isGroup: false,
...
const handleCreateGroup = useCallback(async (): Promise<void> => {
  ...
  try {
    ...
    const groupData = { members: [user.uid,...selected], isGroup: true, groupName: trimmedName, admins: [user.uid],
      createdAt: serverTimestamp(), updatedAt: serverTimestamp(),
      lastMessage: `${user.displayName || "Bạn"} đã tạo nhóm`, lastSenderName: "Hệ thống" };
    ...
    await Promise.all(selected.map(memberId => createNotification(memberId, {
      type: "group_invite", fromUid: user.uid, fromName: userData?.name || "Người dùng",
      fromAvatar: userData?.avatar || "", title: "Mời vào nhóm",
      message: `đã thêm bạn vào nhóm "${trimmedName}"`, groupId: groupRef.id, chatId: groupRef.id,
    })));
    toast.success("Đã tạo nhóm thành công"); router.push(`/chat/${groupRef.id}`);
    setShowAdd(false); setGroupName(""); setSelected([]);
...
const handleDeleteChat = useCallback(async (chat: ChatItem): Promise<void> => {
  ...
  try {
    if (chat.isGroup) {
      ...
      await updateDoc(doc(db, "chats", chat.chatId), { deletedFor: arrayUnion(user.uid), updatedAt: new Date() });
...
const filteredChats = useMemo(() => {
  const query = debounced.toLowerCase().trim(); let filtered = items;
  if (query) {
...
return (
  <>
    <div className="min-h-screen bg-white dark:bg-black select-none">
      ...
      <div className="pb-24">
        {/*...Nội dung danh sách chat, friends, notifications giữ nguyên như file gốc... */}
        {/* (đã rút gọn để part 2 không quá dài - bạn dán nguyên JSX từ file ChatClient.tsx vào đây) */}
      </div>

      {showAdd && (/* modal thêm bạn/tạo nhóm - giữ nguyên */ null)}
      {showScanQR && (/* scanner - giữ nguyên */ null)}
    </div>
  </>
);
} // end ChatClient
...
export default function AppContainer() {
  ...
  useEffect(() => {
    if (db) return;
    try { const _db = getFirebaseDB(); setDb(_db); }
    catch (err) { console.error("Firebase init error:", err); setError("Không thể kết nối database"); setLoading(false); }
  }, [db]);

  //... (tiếp tục buildQuery, loadData trong part 3)
  ...
  const loadData = useCallback(
    async (isRefresh = false) => {
      if (!db) return;
      if (allItems.length > 0) setRefreshing(true);
      else if (isRefresh) setRefreshing(true);
      else setLoading(true);
      const q = buildQuery();
      if (!q) { setLoading(false); setRefreshing(false); return; }
      try {
        ...
        if (err.code === "permission-denied") { setAllItems([]); setHasMore(false); }
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const q = buildQuery(lastDoc); if (!q) return;
      const snap = await getDocs(q);
  ...
  const filteredItems = useMemo(() => {
    let result = [...allItems];
    if (mode === "task") result = result.filter((t) => isTask(t)) as TaskItem[];
    else result = result.filter((t) => isPlan(t)) as PlanItem[];
  ...
  return (
    <LayoutGroup id="app-global-navigation-flow">
      <div className="min-h-screen pb-28 font-sans bg-white dark:bg-zinc-950 select-none relative">
        ...
        {refreshing && <div className="fixed top-0 inset-x-0 h- bg-gradient-to-r from-blue-500 via-purple-500 to-emerald-500 animate-pulse z-50" />}

        <div className="w-full max-w-2xl mx-auto">
          {/* HOME - luôn mount, chỉ ẩn hiện */}
          <div className={currentMainTab === 'home'? 'block' : 'hidden'}>
            <div className="sticky top-0 z-50 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md pt-3 pb-2 px-4 border-b border-gray-100 dark:border-zinc-900">
              <div className="max-w-md mx-auto bg-gray-100 dark:bg-zinc-900 rounded-2xl p-1 flex relative">
                <button onClick={() => { setMode("task"); if ("vibrate" in navigator) navigator.vibrate(5); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-bold rounded-xl transition-all relative z-10 ${mode === "task"? "text-white shadow" : "text-gray-500 dark:text-zinc-400"}`}>
                  <SparklesIcon size={16} /> Task
                  {mode === "task" && <motion.div layoutId="modeSwitchBg" className="absolute inset-0 bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl -z-10" />}
                </button>
                <button onClick={() => { setMode("plan"); if ("vibrate" in navigator) navigator.vibrate(5); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-bold rounded-xl transition-all relative z-10 ${mode === "plan"? "text-white shadow" : "text-gray-500 dark:text-zinc-400"}`}>
                  <CalendarRange size={16} /> Plan
                  {mode === "plan" && <motion.div layoutId="modeSwitchBg" className="absolute inset-0 bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-xl -z-10" />}
                </button>
              </div>
            </div>
              ...
              <div className="max-w-2xl mx-auto px-4">
                <div className="flex justify-around">
                  {subTabs.map((tab) => {
                    ...
                    return (
                      <button key={tab.id} onClick={() => { setActiveTab(tab.id); if ("vibrate" in navigator) navigator.vibrate(5); }}
                        className={`flex flex-col items-center py-3 px-2 flex-1 transition-all active:scale-95 ${active? `text-${tab.color}-600 dark:text-${tab.color}-400` : "text-gray-400 dark:text-zinc-500"}`}>
            ...
            <div className="pt-4">
              ...
              {!loading && hasMore && allItems.length > 0 && (
                <div ref={loadMoreRef} className="px-4 py-6 flex justify-center">
                  {loadingMore && <div className="w-6 h-6 border-2 border-zinc-500 border-t-transparent rounded-full animate-spin" />}
                </div>
              )}
            </div>
          </div>

          {/* MESSAGES - LUÔN MOUNT ĐỂ HẾT LAG */}
          <div className={currentMainTab === 'messages'? 'block' : 'hidden'}>
            <ChatClient />
          </div>

          {/* TASKS */}
          <div className={currentMainTab === 'tasks'? 'block' : 'hidden'}>
            <div className="flex flex-col items-center justify-center pt-32 text-zinc-400">
          ...
          {/* PROFILE */}
          <div className={currentMainTab === 'profile'? 'block' : 'hidden'}>
            <ProfileTabContent onNavigateTab={(tab) => setCurrentMainTab(tab)} />
          </div>
        </div>
            ...
            <AnimatePresence>
              {isMenuOpen && (
                <motion.div initial={{ opacity: 0, y: 15, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                  ...
                  className="w-full bg-white/90 backdrop-blur-2xl rounded- p-2.5 border border-zinc-200/40 shadow-xl pointer-events-auto flex flex-col gap-1">
              ...
              <div className="flex items-center justify-between h-[64px] px-2 relative">
                <div className="flex-1 grid grid-cols-2 h-full">
                  {mainNavItems.slice(0, 2).map((item) => {
                    ...
                    return (
                      <button key={item.id} onClick={() => { setCurrentMainTab(item.id); if ("vibrate" in navigator) navigator.vibrate(10); }}
                        ...
                        {active && <motion.div layoutId="activeIndicatorDot" className={`absolute bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${activeBgClass}`} />}
                      </button>
                    );
                  })}
                </div>
                <div className="w-[64px] flex justify-center h-full items-center relative">
                  <button onClick={() => { setIsMenuOpen(!isMenuOpen); if ("vibrate" in navigator) navigator.vibrate(8); }} className="outline-none z-10 p-2">
                    <motion.div animate={{ rotate: isMenuOpen? 135 : 0 }}
...
// ===== PROFILE TAB - giữ nguyên từ file gốc =====
function ProfileTabContent({ onNavigateTab }: { onNavigateTab: (tab: MainTab) => void }) {
  const db = getFirebaseDB(); const auth = getFirebaseAuth(); const storage = getFirebaseStorage();
  const { user } = useAuth(); const mode = useAppStore((s) => s.mode); const isPlan = mode === "plan";
  const [userData, setUserData] = useState<UserData | null>(null);
  const [name, setName] = useState(""); const [editingName, setEditingName] = useState(false);
  const [uploading, setUploading] = useState(false); const [uploadProgress, setUploadProgress] = useState(0);
  const [showLogoutModal, setShowLogoutModal] = useState(false); const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [showQR, setShowQR] = useState(false); const [showScanQR, setShowScanQR] = useState(false);
  const verifiedRef = useRef(false); const hasCheckedId = useRef(false);
  const uploadTaskRef = useRef<UploadTask | null>(null); const scannerRef = useRef<Html5Qrcode | null>(null);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "users", user.uid), (snap) => {
      if (snap.exists()) {
        ...
        setUserData(data); setName(data.name || "");
        if (user &&!user.emailVerified &&!data.emailVerified &&!verifiedRef.current) {
          verifiedRef.current = true; window.location.href = "/verify-email";
        }
      }
    });
    return () => unsub();
  ...
  useEffect(() => {
    ...
    const createId = async () => {
      hasCheckedId.current = true; let newId = `AIR${nanoid(6).toUpperCase()}`; let attempts = 0;
      while (attempts < 3) {
        const snap = await getDoc(doc(db, "usernames", newId));
        if (!snap.exists()) break; newId = `AIR${nanoid(6).toUpperCase()}`; attempts++;
      }
  ...
  const handleUpdateName = async () => {
    if (!user ||!name.trim() || name.length < 2) { toast.error("Tên tối thiểu 2 ký tự"); return; }
    if (name === userData?.name) { setEditingName(false); return; }
    const oldName = userData?.name; setEditingName(false);
    setUserData((prev) => prev? {...prev, name: name.trim() } : null);
    try { await updateDoc(doc(db, "users", user.uid), { name: name.trim() }); toast.success("Cập nhật tên thành công"); if ("vibrate" in navigator) navigator.vibrate(8); }
    catch { toast.error("Cập nhật thất bại"); setUserData((prev) => prev? {...prev, name: oldName || "" } : null); setName(oldName || ""); }
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const storageRef = ref(storage, `avatars/${user.uid}`); uploadTaskRef.current = uploadBytesResumable(storageRef, file);
      uploadTaskRef.current.on("state_changed", (snapshot) => {
        const prog = (snapshot.bytesTransferred / snapshot.totalBytes) * 100; setUploadProgress(Math.round(prog));
      }, (err) => { if (err.code!== "storage/canceled") toast.error("Upload thất bại"); },
      async () => {
        const task = uploadTaskRef.current; if (!task) return;
        const url = await getDownloadURL(task.snapshot.ref);
        await updateDoc(doc(db, "users", user.uid), { avatar: url });
        toast.success("Cập nhật avatar thành công"); if ("vibrate" in navigator) navigator.vibrate(8); setUploading(false);
      });
    } catch { toast.error("Upload lỗi"); setUploading(false); } finally { e.target.value = ""; }
  };

  const handleShare = async () => {
    if (!userData) return; const url = `https://airanh.vercel.app/u/${userData.userId}`;
    if (navigator.share) { await navigator.share({ title: userData.name || "Người dùng AIR", text: `Kết nối với tôi`, url }); }
    else { navigator.clipboard.writeText(url); toast.success("Đã copy link hồ sơ"); }
    if ("vibrate" in navigator) navigator.vibrate(8);
  };

  const handleLogout = async () => {
    if (!user) return; setShowLogoutModal(false);
  ...
  const handleDeleteAccount = async () => {
    if (!user) return; setShowDeleteModal(false);
    try {
      await Promise.all([deleteDoc(doc(db, "users", user.uid)), deleteDoc(doc(db, "usernames", userData?.userId || ""))]);
      await deleteUser(user); toast.success("Đã xóa tài khoản"); window.location.href = "/register";
    } catch (err: unknown) {
      const error = err as { code?: string };
      if (error.code === "auth/requires-recent-login") {
        toast.error("Vui lòng đăng nhập lại để xóa tài khoản"); await signOut(auth); window.location.href = "/login";
  ...
  useEffect(() => {
    ...
    const startScan = async () => {
      const html5QrCode = new Html5Qrcode("qr-reader"); scannerRef.current = html5QrCode;
      try {
        await html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } },
          (decodedText) => {
            if ("vibrate" in navigator) navigator.vibrate(10); stopScan();
    ...
    <div className="pt-6 pb-12 animate-in fade-in duration-300 bg-white dark:bg-zinc-950">
      <div className="px-6 pb-6">
        <div className="flex items-center gap-4">
          <label className="relative cursor-pointer group flex-shrink-0">
            <img src={userData.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(userData.name)}&size=176&background=8B5E3C&color=fff`} className="w-16 h-16 rounded-full object-cover" alt="Avatar" />
            {userData.emailVerified && <div className={`absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-gradient-to-br ${accentGradient} flex items-center justify-center border-2 border-white dark:border-zinc-950`}><Check className="w-2.5 h-2.5 text-white stroke-[3]" /></div>}
            <div className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-active:opacity-100 transition-opacity"><Camera size={20} className="text-white" /></div>
            <input type="file" accept="image/*" className="hidden" onChange={handleUpload} disabled={uploading} />
            {uploading && <div className="absolute inset-0 bg-black/60 rounded-full flex items-center justify-center backdrop-blur-sm"><span className="text-white text-xs font-bold">{uploadProgress}%</span></div>}
          </label>
          <div className="flex-1 min-w-0">
            {editingName? (
              <div className="flex items-center gap-2">
                <input value={name} onChange={(e) => setName(e.target.value)} onBlur={handleUpdateName} onKeyDown={(e) => e.key === "Enter" && handleUpdateName()} autoFocus className="text-2xl font-extrabold border-b-2 border-gray-300 dark:border-zinc-700 outline-none bg-transparent text-gray-900 dark:text-white flex-1 tracking-tight" />
                <button onClick={handleUpdateName} className={`p-1.5 bg-gradient-to-br ${accentGradient} rounded-full`}><Check size={14} className="text-white" /></button>
              </div>
            ) : (
              <h1 onClick={() => setEditingName(true)} className="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight cursor-pointer leading-tight">{userData.name}</h1>
            )}
            <div className="flex items-center gap-1.5 mt-1">
              <Circle className={`w-2 h-2 fill-current ${userData.online? "text-green-500" : "text-gray-400"}`} />
              <span className="text-sm text-gray-500 dark:text-zinc-400 font-medium">{userData.online? "Đang hoạt động" : "Ngoại tuyến"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowQR(false)}>
          <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <button onClick={handleShare} className="py-3 rounded-2xl font-bold bg-gray-100 dark:bg-zinc-800 text-gray-900 dark:text-white flex items-center justify-center gap-2 active:scale-95 transition"><Share2 size={18} /> Chia sẻ</button>
              <button onClick={() => { setShowQR(false); setShowScanQR(true); }} className={`py-3 rounded-2xl font-bold bg-gradient-to-r ${accentGradient} text-white flex items-center justify-center gap-2 active:scale-95 transition`}><ScanLine size={18} /> Quét mã</button>
            </div>
          </div>
        </div>
      )}
      {showScanQR && (
        <div className="fixed inset-0 bg-black z-50">
          ...
          <button onClick={stopScan} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/50 backdrop-blur flex items-center justify-center"><X className="w-5 h-5 text-white" /></button>
      ...
      {showLogoutModal && <ProfileModal title="Đăng xuất?" desc="Bạn sẽ cần đăng nhập lại để sử dụng app" onClose={() => setShowLogoutModal(false)} onConfirm={handleLogout} confirmText="Đăng xuất" danger />}
      {showDeleteModal && <ProfileModal title="Xóa tài khoản?" desc="Hành động này không thể hoàn tác. Dữ liệu sẽ bị xóa vĩnh viễn." onClose={() => setShowDeleteModal(false)} onConfirm={handleDeleteAccount} confirmText="Xóa vĩnh viễn" danger />}
    </div>
  );
}
...
function ProfileModal({ title, desc, onClose, onConfirm, confirmText, danger }: { title: string; desc: string; onClose: () => void; onConfirm: () => void