...
export default function Home() {
  ...
  // Khởi tạo Firebase bảo đảm chạy mượt
  useEffect(() => {
    if (db) return;
    try {
  ...
  // NÂNG CẤP HOÀN HẢO: Sửa logic load dữ liệu, chặn đứng lỗi treo skeleton và chớp màn hình
  const loadData = useCallback(
    async (isRefresh = false) => {
      ...
      // Nếu đã có dữ liệu từ trước, chuyển sang chế độ làm mờ tải ngầm, KHÔNG bật loading skeleton lại nữa
      ...
      try {
        ...
        // Đổ data mới vào một cục duy nhất tại đây, triệt tiêu khoảng trắng thời gian thực
        setAllItems(data);
        ...
        if (err.code === "permission-denied") {
          ...
          setError("Thiếu cấu hình index database");
        ...
        // Luôn luôn giải phóng hai trạng thái loading để tránh treo giao diện
        setLoading(false);
        setRefreshing(false);
      }
    },
    [db, buildQuery] // Bỏ allItems.length khỏi dependency để tránh loop re-render vô hạn
  );
  ...
  return (
    <div className="min-h-screen pb-24 font-sans bg-gray-50 dark:bg-black select-none">
      ...
      {/* Thanh vạch chạy nhỏ ở đầu màn hình báo hiệu đang update ngầm data mới, tăng tính cao cấp cho UI */}
      {refreshing && (
      ...
      <div className="pt-4">
        ...
        {/* THAY ĐỔI QUYẾT ĐỊNH: Chỉ render Skeleton khi app mở lên mà CHƯA CÓ data.
            Khi đổi mode, giữ nguyên khung feed cũ mờ nhẹ 50% rồi đập data mới vào lập tức, không chớp giật */}
        {loading ? (
          <SkeletonList />
        ) : (
          <div className={`transition-all duration-200 ${refreshing ? "opacity-50 pointer-events-none scale-[0.99]" : "opacity-100 scale-100"}`}>
            <TaskFeed
              tasks={filteredItems}
              mode={mode}
              activeTab={activeTab}
