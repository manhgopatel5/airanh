...
export default function ChatRoom() {
  ...
  const isPublicRoom = typeof chatId === 'string' && chatId.startsWith('public_');
  ...
  useEffect(() => {
    ...
    // 1. Load room data
    const roomRef = doc(db, isPublicRoom? "public_rooms" : "chats", chatId as string);
  ...
  }, [chatId, user?.uid, db, isPublicRoom, router]);

  const handleSendMessage = async () => {
    if (!message.trim() ||!user?.uid ||!chatId || sending) return;
