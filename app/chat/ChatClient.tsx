...
export default function ChatClient() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "unread" | "group" | "friends">("all");
          ...
          <div className="px-4 pt-3 pb-3">
            <div className="flex items-center gap-2.5">
              <div className="relative flex-1">
                ...
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={activeTab === "friends" ? "Tìm bạn bè" : "Tìm kiếm"} className={`w-full h-[38px] pl-[34px] pr-3.5 bg-[#f2f2f7] dark:bg-zinc-900 rounded-[10px] text-[15px] font-normal outline-none border-0 focus:bg-white dark:focus:bg-zinc-900 focus:ring-2 ${primaryRing} transition-all placeholder:text-gray-400`} autoComplete="off" autoCorrect="off" />
              </div>
            ...
            <div className="flex items-center gap-4 mt-3.5 px-0.5 overflow-x-auto scrollbar-hide">
              {[{ key: "all", label: "Tất cả" }, { key: "unread", label: "Chưa đọc" }, { key: "group", label: "Nhóm" }, { key: "friends", label: "Bạn bè" }].map((tab) => (
          ...
          {activeTab === "friends" ? (
            friendsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[60vh] px-8 text-center">
               ...
               <h3 className="text-[20px] font-semibold mb-1.5">
  {search? "Không tìm thấy" : "Chưa có bạn"}
</h3>
<p className="text-[15px] text-[#8e8e93] dark:text-zinc-500 max-w-[280px] leading-[20px]">
