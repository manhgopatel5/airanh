...
const taskDeadline = isTask(task) && task.deadline?.seconds
  ...
  return (
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-white dark:border-zinc-800 shadow-[0_4px_16px_rgba(0,0,0,0.08)] dark:shadow-[0_4px_16px_rgba(0,0,0,0.3)] overflow-hidden mb-3">
  <div className="p-5">
    <div className="flex gap-3 items-start">
      <Link href={`/profile/${task.userId}`} className="relative shrink-0 active:opacity-70 transition-opacity">
        ...
        {owner?.rating && owner.rating >= 4.8 && (
          <div className="absolute -bottom-0.5 -right-0.5 bg-[#00A86B] rounded-full p-0.5">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {!isOwner && (
              <motion.button
                ...
                onClick={handleSave}
                disabled={saving}
                className="w-9 h-9 rounded-xl bg-transparent flex items-center justify-center hover:bg-zinc-100/60 dark:hover:bg-zinc-800/60 active:scale-90 transition-all disabled:opacity-50"
              >
                <FiBookmark 
                  size={18} 
            ...
            <motion.button
              ...
              className="w-9 h-9 rounded-xl bg-transparent flex items-center justify-center hover:bg-zinc-100/60 dark:hover:bg-zinc-800/60 active:scale-90 transition-all"
            >
              <FiShare2 size={18} className="text-zinc-600 dark:text-zinc-300" strokeWidth={2} />
            </motion.button>
            ...
            <div className="relative">
              <motion.button
                ...
                className="w-9 h-9 rounded-xl bg-transparent flex items-center justify-center hover:bg-zinc-100/60 dark:hover:bg-zinc-800/60 active:scale-90 transition-all"
              >
                <FiMoreHorizontal size={18} className="text-zinc-600 dark:text-zinc-300" strokeWidth={2.5} />
              </motion.button>
              <AnimatePresence>
                {showMenu && (
                  <Portal>
                    ...
                    <motion.div
                      ...
                      className="fixed z-50 min-w- bg-white dark:bg-zinc-900 rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.5)] ring-1 ring-black/5 dark:ring-white/10 py-2 overflow-hidden"
                      style={{
                        top: `${menuPos.y}px`,
                        left: `${menuPos.x}px`,
                      }}
    ...
    <div className="flex items-center gap-1.5 mt-3 flex-nowrap overflow-hidden">
      <span className={`flex-1 min-w-0 px-2 py-1 rounded-xl text- font-semibold flex items-center justify-center gap-1 ${status.color}`}>
        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${status.dot}`} />
        <span className="truncate">{status.label}</span>
      </span>

      {isTask(task) && (
        <span className="flex-1 min-w-0 px-2 py-1 rounded-xl text- font-semibold bg-[#E8F0FE] text-[#1A73E8] dark:bg-[#1A73E8]/20 dark:text-[#8AB4F8] flex items-center justify-center gap-1">
      ...
      {isTask(task) && task.price > 0 && (
        <span className="flex-1 min-w-0 px-2 py-1 rounded-xl text- font-semibold bg-[#E3F2FD] text-[#0A84FF] dark:bg-[#0A84FF]/20 dark:text-[#5AC8FA] flex items-center justify-center">
          <span className="truncate">{task.price.toLocaleString("vi-VN")} đ</span>
        </span>
      )}

      {isTask(task) && task.deadline?.seconds && task.status!== "completed" && (
        <span className={`flex-1 min-w-0 px-2 py-1 rounded-xl text- font-semibold flex items-center justify-center ${
          isUrgent 
    ...
    <div className="flex items-center gap-2 mt-3">
      <div className="flex-1 px-1.5 py-2 rounded-xl bg-[#F2F2F7] dark:bg-zinc-800/60 border border-[#E5E5E7] dark:border-zinc-700">
        <div className="flex items-center justify-center gap-1">
          <FiCalendar size={13} className="shrink-0 text-[#8E8E93]" />
      ...
      <div className="flex-1 px-1.5 py-2 rounded-xl bg-[#FFE5E5] dark:bg-[#FF3B30]/10 border border-[#FECACA] dark:border-[#FF3B30]/30">
        <div className="flex items-center justify-center gap-1">
          <FiClock size={13} className="shrink-0 text-[#FF3B30]" />
          <div className="text-center">
    ...
    {!isOwner && (
      <div className="grid grid-cols-4 gap-2 mt-3">
        <motion.button
          ...
          disabled={isOwner}
          className="h-12 rounded-xl bg-[#F2F2F7] dark:bg-zinc-800 flex flex-col items-center justify-center gap-0.5 text-[#1C1C1E] dark:text-zinc-100 active:bg-[#E5E5EA] dark:active:bg-zinc-700 disabled:opacity-40 transition-all"
        >
        ...
        >
          ...
          <span className="text- font-medium">Gọi điện</span>
        </motion.button>
        ...
        <motion.button
          ...
          className={`h-12 rounded-xl flex flex-col items-center justify-center gap-0.5 font-semibold active:scale-95 disabled:opacity-40 transition-all ${
            isApplied
        ...
        >
          ...
          <span className="text-">{isApplied? "Đã ứng tuyển" : "Ứng tuyển"}</span>
        </motion.button>

        <motion.button
          whileTap={{ scale: 0.94 }}
          onClick={() => toast.info("Đã gửi báo cáo")}
          className="h-12 rounded-xl bg-[#F2F2F7] dark:bg-zinc-800 flex flex-col items-center justify-center gap-0.5 text-[#FF9500] active:bg-[#E5E5EA] dark:active:bg-zinc-700 transition-all"
        >
          <FiAlertTriangle size={20} strokeWidth={2} />
          <span className="text- font-medium">Báo cáo</span>
        </motion.button>
      </div>
    )}
  </div>
