...
export default function BookmarksPage() {
  ...
  // Load bookmarks - fix: cleanup + batch getDoc
  useEffect(() => {
    if (!user?.uid) return;
    setLoading(true);
    ...
    unsubRef.current = onSnapshot(q, async (snap) => {
      try {
        ...
        // Batch get để tránh spam Firestore
        const chunks: string[][] = [];
        for (let i = 0; i < taskIds.length; i += BATCH_SIZE) {
          chunks.push(taskIds.slice(i, i + BATCH_SIZE));
        }
        ...
        const allTasks = taskArrays.flat().sort(
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-black pb-24">
      ...
      <div className="sticky top-0 z-40 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-2xl border-b border-black/5 dark:border-white/10">
        <div className="max-w-[720px] mx-auto px-4 h-[52px]">
          <div className="flex items-center gap-3 h-full">
            <Link
              href="/"
              className="w-9 h-9 -ml-1 grid place-items-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 active:scale-90 transition-all"
            >
              <FiArrowLeft size={22} className="text-[#0a84ff]" />
            </Link>
            <div className="flex-1">
              <h1 className="text-[17px] font-bold leading-tight">Đã lưu</h1>
              <p className="text-[13px] text-[#8e8e93] leading-tight">
                {stats.total} mục • {stats.tasks} việc • {stats.plans} kế hoạch
              </p>
            </div>
            <button
              onClick={() => {
              ...
              className="w-9 h-9 grid place-items-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 active:scale-90 transition-all"
            >
              {viewMode === "list"? (
                <FiGrid size={18} className="text-[#8e8e93]" />
              ) : (
                <FiList size={20} className="text-[#8e8e93]" />
              )}
            </button>
          </div>
        </div>
      ...
      <div className="max-w-[720px] mx-auto px-4 py-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-3 bg-white dark:bg-zinc-900 rounded-xl outline-none text-[15px] border border-black/5 dark:border-white/10 focus:border-[#0a84ff]/50 focus:ring-4 focus:ring-[#0a84ff]/10 transition-all"
            />
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
            {[
              ...
              <button
                ...
                className={`h-8 px-3.5 rounded-full text-[15px] font-medium whitespace-nowrap transition-all active:scale-95 ${
                  filter === tab.id
                   ? "bg-[#0a84ff] text-white shadow-lg shadow-[#0a84ff]/20"
                    : "bg-white dark:bg-zinc-900 text-[#3a3a3c] dark:text-zinc-300 hover:bg-black/5 dark:hover:bg-white/5 border border-black/5 dark:border-white/10"
                }`}
              >
                {tab.label} <span className={filter === tab.id? "opacity-80" : "opacity-60"}>({tab.count})</span>
              </button>
            ))}
          </div>
        </motion.div>
          ...
          {loading? (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              {Array.from({ length: 4 }).map((_, i) => (
                <motion.div
                  ...
                  className="bg-white dark:bg-zinc-900 rounded-3xl p-4 border border-black/5 dark:border-white/5"
                >
              ...
              <div className="flex justify-center pt-4">
                <LottiePlayer animationData={L.loadingPull} loop autoplay className="w-12 h-12 opacity-60" />
              </div>
            </motion.div>
            ...
            >
              ...
              >
                ...
                <div className="relative w-full h-full bg-white dark:bg-zinc-900 rounded-full grid place-items-center shadow-xl border border-black/5 dark:border-white/10">
                  <FiBookmark className="text-[#8e8e93]" size={40} strokeWidth={1.5} />
                </div>
              </motion.div>

              <h2 className="text-xl font-bold text-black dark:text-white mb-2">
                {search || filter!== "all"? "Không tìm thấy" : "Chưa có mục nào"}
              </h2>
              <p className="text-[15px] text-[#8e8e93] max-w-xs mx-auto leading-relaxed mb-8">
                {search || filter!== "all"
              ...
              {!search && filter === "all" && (
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Link
                    href="/"
                    className="inline-flex items-center gap-2 px-6 h-12 rounded-2xl text-white font-semibold bg-[#0a84ff] shadow-lg shadow-[#0a84ff]/25 active:shadow-md transition-all"
                  >
              ...
              {(search || filter!== "all") && (
                <button
                  ...
                  className="px-5 h-10 rounded-xl bg-white dark:bg-zinc-900 font-medium text-[#0a84ff] border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 active:scale-95 transition-all"
                >
                  Xóa bộ lọc
                </button>
              )}
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className={viewMode === "grid"? "grid grid-cols-1 sm:grid-cols-2 gap-3" : "space-y-3"}
            >
              <AnimatePresence>
        ...
        {!loading && filteredTasks.length > 0 && (
          <motion.div
            ...
            className="mt-8 pt-6 border-t border-black/5 dark:border-white/5"
          >
