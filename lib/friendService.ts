...
export const sendFriendRequest = async (from: string, to: string): Promise<void> => {
  ...
  if (countSnap.data().count >= 20) {
    throw new FriendError("Bạn đã gửi quá 20 lời mời hôm nay", "LIMIT_EXCEEDED");
  }
  ...
  if (reqSnap.exists()) {
    ...
    if (status === "rejected") {
      ...
      await updateDoc(toUserRef, { friendRequestsUnread: increment(1) });
      return;
    }
  }
  ...
  batch.update(toUserRef, { friendRequestsUnread: increment(1) });
  await batch.commit();
};
...
export const getFriendshipStatus = async (
  ...
  const [friendSnap, reqSent, reqReceived, block1, block2] = await Promise.all([
    ...
    getDoc(doc(db, "friendRequests", [user1, user2].sort().join("_"))),
    getDoc(doc(db, "friendRequests", [user2, user1].sort().join("_"))),
  ...
  if (reqSent.exists() && reqSent.data().status === "pending") return "pending_sent";
  if (reqReceived.exists() && reqReceived.data().status === "pending") return "pending_received";
  return "none";
};
