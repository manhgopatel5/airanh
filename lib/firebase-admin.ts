...
.where('type', '==', type) // ← Dùng param type
...
.get();
  ...
  type TaskDoc = Record<string, any> & { id: string };
  const docs: TaskDoc[] = snap.docs
.map(doc => ({ id: doc.id,...doc.data() } as TaskDoc))
.filter(d => d.banned!== true && d.hidden!== true)
.slice(0, limitCount);
